...
if (document.getElementById('formEditarVacante')) {
    document.getElementById('formEditarVacante').addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch(`https://zomedica.onrender.comvacantes/${vacanteId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
...
async function handleUrlParams() {
    ...
    if (verifyToken) {
        try {
            const response = await fetch(`https://zomedica.onrender.com/verify-email/${verifyToken}`);
            const data = await response.json();
            if (response.ok) {
...
async function verPerfilPostulante(postulacionId) {
    ...
    try {
        ...
        const perfil = await res.json();
        const imagenSrc = perfil.fotoPath ? `https://zomedica.onrender.comperfil.fotoPath}` : 'uploads/default-avatar.png';
